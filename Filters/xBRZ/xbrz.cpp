...
namespace
{
template <u_int N> inline
unsigned char getByte(u_int val) { return static_cast<unsigned char>((val >> (8 * N)) & 0xff); }

inline unsigned char getRed  (u_int val) { return getByte<2>(val); }
inline unsigned char getGreen(u_int val) { return getByte<1>(val); }
inline unsigned char getBlue (u_int val) { return getByte<0>(val); }
...
const u_int redMask   = 0xff0000;
const u_int greenMask = 0x00ff00;
const u_int blueMask  = 0x0000ff;

template <unsigned int N, unsigned int M> inline
void alphaBlend(u_int& dst, u_int col) //blend color over destination with opacity N / M
{
...
inline
u_int alphaBlend2(u_int pix1, u_int pix2, double alpha)
{
    return (redMask   & static_cast<u_int>((pix1 & redMask  ) * alpha + (pix2 & redMask  ) * (1 - alpha))) |
           (greenMask & static_cast<u_int>((pix1 & greenMask) * alpha + (pix2 & greenMask) * (1 - alpha))) |
           (blueMask  & static_cast<u_int>((pix1 & blueMask ) * alpha + (pix2 & blueMask ) * (1 - alpha)));
}


u_int*       byteAdvance(      u_int* ptr, int bytes) {  return reinterpret_cast<      u_int*>(reinterpret_cast<      char*>(ptr) + bytes); }
const u_int* byteAdvance(const u_int* ptr, int bytes) {  return reinterpret_cast<const u_int*>(reinterpret_cast<const char*>(ptr) + bytes); }


//fill block  with the given color
inline
void fillBlock(u_int* trg, int pitch, u_int col, int blockWidth, int blockHeight)
{
...
inline
void fillBlock(u_int* trg, int pitch, u_int col, int n) { fillBlock(trg, pitch, col, n, n); }
...
{
public:
    OutputMatrix(u_int* out, int outWidth) : //access matrix area, top-left at position "out" for image with given width
        out_(out),
        outWidth_(outWidth) {}
...
/*
inline
void rgbtoLuv(u_int c, double& L, double& u, double& v)
{
...
inline
void rgbtoLab(u_int c, unsigned char& L, signed char& A, signed char& B)
{
...
inline
double distLAB(u_int pix1, u_int pix2)
{
...
/*
inline
void rgbtoHsl(u_int c, double& h, double& s, double& l)
{
...
inline
double distHSL(u_int pix1, u_int pix2, double lightningWeight)
{
    double h1 = 0;
    double s1 = 0;
    double l1 = 0;
...
inline
double distRGB(u_int pix1, u_int pix2)
{
...
inline
double distNonLinearRGB(u_int pix1, u_int pix2)
{
...
inline
double distYCbCr(u_int pix1, u_int pix2, double lumaWeight)
{
...
inline
double distYUV(u_int pix1, u_int pix2, double luminanceWeight)
{
...
inline
double colorDist(u_int pix1, u_int pix2, double luminanceWeight)
{
...
BlendResult preProcessCorners(const Kernel_4x4& ker, const xbrz::ScalerCfg& cfg) //result: F, G, J, K corners of "GradientType"
{
    ...
    auto dist = [&](u_int col1, u_int col2) { return colorDist(col1, col2, cfg.luminanceWeight_); };
...
#define DEF_GETTER(x) template <RotationDegree rotDeg> u_int inline get_##x(const Kernel_3x3& ker) { return ker.x; }
...
#define DEF_GETTER(x, y) template <> inline u_int get_##x<ROT_90>(const Kernel_3x3& ker) { return ker.y; }
...
#define DEF_GETTER(x, y) template <> inline u_int get_##x<ROT_180>(const Kernel_3x3& ker) { return ker.y; }
...
#define DEF_GETTER(x, y) template <> inline u_int get_##x<ROT_270>(const Kernel_3x3& ker) { return ker.y; }
...
#define i get_i<rotDeg>(ker)
    ...
    {
        auto eq   = [&](u_int col1, u_int col2) { return colorDist(col1, col2, cfg.luminanceWeight_) < cfg.equalColorTolerance_; };
        auto dist = [&](u_int col1, u_int col2) { return colorDist(col1, col2, cfg.luminanceWeight_); };
        ...
        const u_int px = dist(e, f) <= dist(e, h) ? f : h; //choose most similar color
...
void scaleImage(const u_int* src, u_int* trg, int srcWidth, int srcHeight, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
{
    ...
    //"sizeof(u_int) * srcWidth * (yLast - yFirst)" bytes without risk of accidental overwriting before accessing
    ...
    if (yFirst > 0)
    {
        ...
        const u_int* s_m1 = src + srcWidth * std::max(y - 1, 0);
        const u_int* s_0  = src + srcWidth * y; //center line
    ...
    {
        u_int* out = trg + Scaler::scale * y * trgWidth; //consider MT "striped" access
        ...
        for (int x = 0; x < srcWidth; ++x, out += Scaler::scale)
        {
            ...
            fillBlock(out, trgWidth * sizeof(u_int), s_0[x], Scaler::scale); //place *after* preprocessing step, to not overwrite the results while processing the the last pixel!
...
struct Scaler2x
{
    ...
    static void blendLineShallow(u_int col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineSteep(u_int col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineSteepAndShallow(u_int col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineDiagonal(u_int col, OutputMatrix& out)
    {
        alphaBlend<1, 2>(out.template ref<1, 1>(), col);
    }

    template <class OutputMatrix>
    static void blendCorner(u_int col, OutputMatrix& out)
    {
...
void xbrz::scale(size_t factor, const u_int* src, u_int* trg, int srcWidth, int srcHeight, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
{
    switch (factor)
    {
        case 2:
...
bool xbrz::equalColor(u_int col1, u_int col2, double luminanceWeight, double equalColorTolerance)
{
    return colorDist(col1, col2, luminanceWeight) < equalColorTolerance;
}


void xbrz::nearestNeighborScale(const u_int* src, int srcWidth, int srcHeight, int srcPitch,
                                u_int* trg, int trgWidth, int trgHeight, int trgPitch,
                                SliceType st, int yFirst, int yLast)
{
    if (srcPitch < srcWidth * static_cast<int>(sizeof(u_int))  ||
        trgPitch < trgWidth * static_cast<int>(sizeof(u_int)))
    {
        assert(false);
        return;
    }
    ...
    switch (st)
    {
        case NN_SCALE_SLICE_SOURCE:
            ...
            for (int y = yFirst; y < yLast; ++y)
            {
                ...
                if (blockHeight > 0)
                {
                    const u_int* srcLine = byteAdvance(src, y * srcPitch);
                    u_int* trgLine  = byteAdvance(trg, yTrg_first * trgPitch);
                    int xTrg_first = 0;
        ...
        case NN_SCALE_SLICE_TARGET:
            ...
            for (int y = yFirst; y < yLast; ++y)
            {
                u_int* trgLine = byteAdvance(trg, y * trgPitch);
                const int ySrc = srcHeight * y / trgHeight;
                const u_int* srcLine = byteAdvance(src, ySrc * srcPitch);
