...
void Diagram::createDiagram( SimilarityGraph* graph )
{
    ...
    for( int i = 0; i < _height; i++ )
    {
        for( int j = 0; j < _width; j++ )
        {
            ...
            _cells[ index ]->createCellFromPattern( graph->getNodeIndex( index ), graph->getNodeIndex(
                                                        indexLeft ), graph->getNodeIndex( indexRight ) );
        }
    }
}
...
vector< Point2D > Diagram::extractFirstCellEdges( SimilarityGraph* graph, vector< Point2D > cellPoints, int index,
                                                  int width )
{
    ...
    for( int i = lowestIndex; i > ( teste ); i-- )
    {
        if( !isIntersected( cellPoints.at( ( i + cellPoints.size() ) % cellPoints.size() ),
                            cellPoints.at( ( i - 1 + cellPoints.size() ) % cellPoints.size() ),
                            graph->getNodeIndex( index ).value,
                            index, width ) )
        {
            ...
            if( hasTJunction )
            {
                ...
                {
                    if( getNormalizedPoint( _TJunctionPoints.at( j ), index,
                                            width ) == cellPoints.at( ( i + cellPoints.size() ) % cellPoints.size() ) )
                    {
...
vector< Point2D > Diagram::extractCellEdges( SimilarityGraph* graph, vector< Point2D > cellPoints, Point2D lastPoint,
                                             int index,
                                             int width )
{
    vector< Point2D > cEdgePoints;
    ...
    for( int i = 0; i < _height; i++ )
    {
        for( int j = 0; j < _width; j++ )
        {
            ...
            if( graph->isIslandNode( node ) )
            {
                ...
                islandSpline.push_back( getNormalizedPoint( getCells()[ index ]->getPoints()[ getCells()[ index ]->
                                                                                              getN_points() - 1 ],
                                                            index, _width ) );
            ...
            if( it == listed.end() )
            {
                ...
                while( ( c_neighbor_index( index, edge,
                                           _width ) != firstNodeCC ) || ( connected_edge( edge ) != edgeOfArrival ) )
                {
